 ...
 */
  ...
  */
  function getHellowWorldDocument(db, id, callback) {
    ...
    db.collection('helloworld').findOne(query, function(err, doc) {
      if (err) {
        ...
        // Success! If we found the document, then doc contains the document. If we did not find the document, doc is null.
        callback(doc);
      }
    });
  }
...
function mongoExample(db) {
  ...
  insertExample(db, function(newId) {
    ...
    getHellowWorldDocument(db, newId, function(doc) {
